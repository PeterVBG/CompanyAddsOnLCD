...
hd44780::hd44780() {
    ...
    */
    ...
    //HUVUDFOKUS: SÄTT AKTUELLA PINNAR TILL ATT VARA OUT-PORTS
    //Det är pinnarna på Arduinokortet vi definierar här. Alla pinnar finns i port D
    //Alla pinnar i Port D ska vara out-ports
    //Vi använder macrot LCD_RS_DIR istället för DDRD, se defines i .hpp
    LCD_RS_DIR |= LCD_RS; //PD2 ska vara en output port. PD2 är pinne 2 i Port D
    LCD_E_DIR |= LCD_E; //I klartext: DDRD |= (1 << PD3)
    LCD_DB4_DIR |= LCD_DB4; 
    LCD_DB5_DIR |= LCD_DB5; 
    LCD_DB6_DIR |= LCD_DB6; 
    LCD_DB7_DIR |= LCD_DB7; 
    ...
    */
    ...
    //HUVUDFOKUS: SÄTT PINNE 2 OCH 3 I PORT D TILL LOW
    //Nu ska vi sätta pinnarna LCD_E (PD3, se .hpp) och LCD_R (PD2, se .hpp) till LOW. 
    //Vi använder LCD_E_PORT istället för PORTD, se defines i .hpp
    LCD_RS_PORT &= ~LCD_RS; //i klartext: PORTD &= ~(1 << PD2)
    LCD_E_PORT &= ~LCD_E; //i klartext: PORTD &= ~(1 << PD3)
...
void hd44780::blink_text(const char *text) {
    // Simple blink: show text, then clear and re-show a few times
    if (!text) return;

    // Show the text initially
    write_text(text);

    //PETER: Changed 3 to 13
    // Blink 3 times: clear -> wait -> show -> wait
    for (int i = 0; i < 13; ++i) {
        millis_wait_ms(250);
        clear();
        millis_wait_ms(250);
